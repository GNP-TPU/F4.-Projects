...
void SystemCoreClockConfigure(RCC_InitTypeDef* RCC_Init){
	...
	if(RCC_Init->MCO1_Enable){
		...
		RCC->CFGR |= RCC_Init->MCO1_Prescaler << 24;
		RCC->CFGR |= RCC_Init->MCO1_Source << 21;
	}
	*/
	...
	switch((uint32_t)RCC_Init->SystemClockSource){
		...
		case PLL_CLK_SRC:
			...
			if((RCC_Init->HCLK_Clock > 64000000) && (RCC_Init->HCLK_Clock <= 84000000))
				PWR->CR |= PWR_CR_VOS_1;
			if((RCC_Init->HCLK_Clock > 84000000) && (RCC_Init->HCLK_Clock <= 100000000))
				PWR->CR |= PWR_CR_VOS;
			else
				PWR->CR |= PWR_CR_VOS_0;
