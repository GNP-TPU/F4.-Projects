...
static void RC522_SPI_Transmit(uint8_t data){
	
}

static uint8_t RC522_SPI_Receive(void){
	uint8_t byte = 0;
...
//===============================================================================
void RC522_Read_PICC_Data(RC522_TypeDef* RC522_Data){
...
void RC522_Write_PICC_Data(RC522_TypeDef* RC522_Data){
	uint8_t BlockNumber 		= RC522_Data->Block + (RC522_Data->Sector * 4);
	uint8_t SectorKeyBlock	= (RC522_Data->Sector * 4) + 3;
...
void RC522_SetUID(RC522_TypeDef* RC522_Data){
	uint8_t BlockNumber 		= RC522_Data->Block + (RC522_Data->Sector * 4);
	uint8_t SectorKeyBlock	= (RC522_Data->Sector * 4) + 3;
	...
	if(sta == MI_OK){
		...
		if(sta == MI_OK){
			...
			//if(sta > 0){
				SectorKeyBlock = (RC522_Data->Sector * 4) + 3;
				sta = RC522_Auth(RC522_Data->PICC_AuthMode, SectorKeyBlock, KeyB, cardstr);

				sta = RC522_ReadBlock(BlockNumber, str);

				for(uint8_t count = 0; count < 16; count++){
					RC522_Data->PICC_ReadData[count] = str[count];
		...
		if((buff[3] == 0x0A)){
			sta = RC522_WriteBlock(BlockNumber, RC522_Data->PICC_WriteData);
			if(sta == MI_OK){
				RC522_Data->Status = 0x02;
			}
			else{
				RC522_Data->Status = 0x03;
			}
		}
		else{
			RC522_Data->Status = 0x03;
...
//==============================================================================
void RC522_Init(RC522_TypeDef*){
	RC522_RST_LOW();
	for(volatile long int i = 0; i < 1000000; i++);
	RC522_RST_HIGH();
	for(volatile long int i = 0; i < 1000000; i++);
	...
	RC522_ReadRegister(0x37); // Read RC522 version
}
