...
void ST77xx_Init(void)
{
  ...
  delay_ms(100);
	...
	delay_ms(500);
	ST77xx_FillScreen(YELLOW);
	delay_ms(500);
	ST77xx_FillScreen(GREEN);
	delay_ms(500);
	ST77xx_FillScreen(WHITE);
	delay_ms(500);
...
void ST77xx_WriteChar(char ch, uint16_t x, uint16_t y, uint16_t color, uint16_t bgcolor){
	uint32_t b;
	
	ST77xx_SetWindow(x, y, x + 16 - 1, y + 26 - 1);
	...
	for(uint32_t i = 0; i < 26; i++){
		...
		for(uint32_t j = 0; j < 16; j++){
			if((b << j) & 0x8000){
				...
				ST77xx_RamWrite(bgcolor, 1);
			}
		}
	}
}

void ST77xx_WriteString(char* str, uint16_t x, uint16_t y, uint16_t color, uint16_t bgcolor){
	while(*str){
		if((x + 16) >= ST77xx_Width){
			x = 0;
			y += 26;
			if((y + 26) >= ST77xx_Height){
				break;
			}
			if(*str == ' '){
				str++;
		...
		ST77xx_WriteChar(*str, x, y, color, bgcolor);
		x += 16;
		str++;
	}
}
